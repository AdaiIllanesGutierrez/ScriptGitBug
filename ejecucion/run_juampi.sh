...
poetry run python "$SCRIPT_DIR/run_screened_bugs.py" \
  ...
  --evosuite-jar "$PROJECT_ROOT/evosuiteJUAMPI/master/target/evosuite-master-1.2.1-SNAPSHOT.jar" \
  --bugs-csv "$BASE_DIR/data/bugs_java_assigned.csv" \
  --seeds 3147383999447 9617663486099 5379124608314 \
  --workers 3 --cleanup-targets \
  --limit 3 \
  --work-root "$PROJECT_ROOT/resultados/JUAMPI" \
